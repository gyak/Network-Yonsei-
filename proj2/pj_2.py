...
class FileTransfer:
    def __init__(self) -> None:
        self.file_pointer = None #파일을 read/write 하기 위한 객체
        self.udp_recv_packet = [bytes(PACKET_SIZE) for _ in range(UDP_MAX_ACK_NUM)] #수신한 패킷을 저장하는 리스트, 파일을 순서대로 재조립하기 위해서 사용
        self.udp_recv_flag = [False for _ in range(UDP_MAX_ACK_NUM)] #패킷 수신 여부를 기록하기 위한 리스트
        self.udp_send_packet = dict() #현재 윈도우 만큼의 패킷을 저장하고 있는 딕셔너리, key: packet number, Value: (전송 time, packet)
        self.udp_ack_windows = [False for _ in range(UDP_MAX_ACK_NUM)] #현재 윈도우 만큼의 패킷에 대한 ACK 수신 여부를 기록하기 위한 리스트
        self.udp_ack_num = 0 #현재 윈도우에서 가장 첫번째에 위치한 패킷의 시퀀스 넘버 -> send_base
        self.udp_last_ack_num = 0 #다음 전송해야 할 패킷의 시퀀스 넘버 -> nextseqnum
        self.file_packet_start = 0 #연속적인 가장 마지막 패킷 번호 1,2,4,5,6 받은 경우에 2를 뜻함
        self.file_name = None #전송하고자 하는 파일 이름


    @staticmethod
    def tcp_packet_pack(packet_type: bytes, data: bytes) -> bytes: #전송할 데이터를 패킷으로 변환, 리턴: packet 
        data_len = len(data)
    ...
    @staticmethod
    def tcp_packet_unpack(packet: bytes) -> Tuple[bytes, bytes]: #수신한 패킷을 데이터로 변환, 리턴: packet_type, data
    ...
    @staticmethod
    def udp_packet_pack(packet_type: bytes, ack_num: Any, data: bytes) -> bytes: #전송할 데이터를 패킷으로 변환, 리턴: packet 
        data_len = len(data)
        if type(ack_num) == int:
    ...
    @staticmethod
    def udp_packet_unpack(packet: bytes) -> Tuple[bytes, int, bytes]: #수신한 패킷을 데이터로 변환, 리턴: packet_type, data
        packet_type = packet[:1]
    ...
    @staticmethod
    def udp_ack_bytes(packet: bytes) -> bytes: #수신한 패킷에서 ACK number 반환, 리턴: ACK number
        return packet[1:3]

    def tcp_file_name_packet(self, file_name: str) -> bytes: #파일 이름을 전송하기 위한 패킷 생성, 리턴: 파일 이름이 담긴 패킷
        # TCP 통신에서의 파일 이름 전송용 패킷 생성 
        # 패킷 구조: \x00 + (이름 data 크기) + (파일 이름 data)
        data = file_name.encode(ENCODING)
        return self.tcp_packet_pack(PACKET_TYPE_FILE_START, data)

    
    def tcp_file_data_packet(self) -> Tuple[bool, bytes]: #파일을 구성하는 데이터를 읽고 패킷 단위로 생성하여 전송, 리턴: 데이터 존재 여부, packet
    ...
    def udp_file_data(self) -> Tuple[bool, bytes]: #현재 전송할 데이터 찾기, 리턴: 데이터 존재 여부, data
        # udp sener가 전송할 파일 data를 얻는다
        # 결과값: 파일 data
    ...
    def udp_file_name_transfer(self, file_name: str, udp_send_func: Callable)-> None: #파일 이름을 전송
        data = file_name.encode(ENCODING)
    ...
    def udp_file_receive(self, packet: bytes, udp_send_func: Callable) -> int: #파일 수신 및 저장
        ...
        if packet_type == PACKET_TYPE_FILE_START:  # file transfer start
            ...
            while self.udp_recv_flag[check] == True:
                self.file_pointer.write(self.udp_recv_packet[check])
                self.udp_recv_flag[check] = False
    ...
    def udp_time_out(self) -> bool: #전송한 패킷의 timeout 여부 체크, 리턴: 타임아웃 여부
    ...
    def udp_ack_send(self, ack_bytes:bytes, udp_send_func: Callable): #수신한 패킷에 대한 ACK 전송
